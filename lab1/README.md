...
Хранит всех участников платформы. Один пользователь может быть как владельцем (выставляет инвентарь), так и арендатором (берёт в аренду) — роль определяется контекстом использования.
...
**Обоснование:** `email` — `VARCHAR(255)` по максимальной длине emai. `rating` — `NUMERIC(3,2)` для точного хранения дробных значений без погрешностей float.

---

### `equipment_categories` — Категории инвентаря

Справочник для нормализации: категории вынесены отдельно, чтобы не дублировать строки в `equipment`. Это переводит схему в 3NF.
...
**Обоснование:** Вынесение категорий устраняет транзитивную зависимость `equipment.category_name → equipment.category_description`, обеспечивая 3NF.
...
**Обоснование:** `price_per_day` — `NUMERIC(10,2)` исключает ошибки округления при денежных расчётах. `is_available` — `BOOLEAN` для быстрой фильтрации доступного инвентаря.
...
**Обоснование:** Отделение платежей от аренды устраняет повторяющиеся группы (1NF) и позволяет корректно обрабатывать частичные возвраты. `paid_at` — `TIMESTAMP` (с временем, не только датой) для точного логирования.
...
**Обоснование:** `rating` — `SMALLINT` (2 байта) вместо `INTEGER` — достаточен для диапазона 1–5, экономит место. `UNIQUE` на `rental_id` — один отзыв на одну аренду.
...
```text
├── scripts/
│   ├── er_diagram.puml   # Исходник ER-диаграммы (PlantUML)
│   └── DDL.sql           # Создание таблиц
├── images/
│   └── er_diagram.png    # Экспортированная диаграмма
└── README.md
```
...
**Что не индексируем:**
- `payments(rental_id)` — платежи ищутся только в связке с конкретной арендой, таблица мала
- `reviews.rental_id` — уже покрыт неявным индексом от `UNIQUE`-ограничения
...
| **2NF** | Все не-ключевые атрибуты зависят от всего первичного ключа (все PK — суррогатные `SERIAL`) |
